...
function extra_user_profile_fields( $user ) { ?>
    <h3><?php _e("Banco de imagens:", "blank"); ?></h3>

    <table class="form-table">
    <tr>
        <th><label for="drive"><?php _e("Drive de fotos"); ?></label></th>
        <td>
            <input type="text" name="drive" id="drive" value="<?php echo esc_attr( get_the_author_meta( 'drive', $user->ID ) ); ?>" class="regular-text" /><br />
            <span class="description"><?php _e("Link banco de fotos do cliente."); ?></span>
        </td>
    </tr>
    </table>
<?php }
...
function save_extra_user_profile_fields( $user_id ) {
    ...
    update_user_meta( $user_id, 'drive', $_POST['drive'] );
}

function title_limit($max){
	$title = get_the_title();
	if (strlen( $title ) > $max){
		$excerpt = '...';
	}
